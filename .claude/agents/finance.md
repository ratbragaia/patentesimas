...
Fiscal parameters are fixed in ADR 0002 and `src/lib/company.ts` (LC 116 item 1.09, export, ISS 0, PIS/COFINS exempt). The municipal `NFEIO_CITY_SERVICE_CODE` comes from NFe.io's São João del Rei table at onboarding; until it is set, invoices stay `pending` with that error and you do not guess.
