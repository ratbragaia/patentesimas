...
| 13 | Postmark server token, verified sender domain (DKIM/Return-Path) | `POSTMARK_*` | 🟡 token stored 2026-09-26 (server `PatentSonar`, Free plan; streams `outbound`/`broadcast`/`inbound`). Pending: domain verification (DKIM + Return-Path CNAMEs, merged SPF) |
