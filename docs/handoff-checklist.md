...
| 12 | ~~NFe.io~~ **Notaas** account (free tier, 50 NFS-e/month; research 06) + **e-CNPJ A1 certificate** uploaded there; access to nfse.gov.br with the same certificate as fallback | `NOTAAS_API_KEY` | 🟡 key received and stored 2026-09-26 (valid). Pending: confirm the key's Notaas project is CNPJ 40.435.866/0001-40 in **homologação** (ambiente is a project setting) before the sandbox test; then production switch + A1 certificate |
