...
| Service code | **LC 116 item 1.09**; national code (`cTribNac`) 010901, confirmed in the national table at onboarding; fallback 17.01 if 1.09 is unavailable | research 05 §2, research 06 |
...
- `cli invoices enqueue-payout <payout_id> <UK|US|IE> <usd> <YYYY-MM-DD> [reverse_invoice_ref]` records a
  payout; the daily `invoices issue` timer converts at PTAX and sends a provider-neutral draft (national DPS
  vocabulary: tomador no exterior, exigibilidade "exportação", ISS 0, bloco de comércio exterior em USD) through
  **Notaas** (`src/invoicing/nfse.ts`). **Channel (research 06):** São João del Rei issues NFS-e only through the
  national system since 2026-01-01; NFe.io (R$190/month) was dropped for Notaas' free tier (50 notes/month;
  we need 1–2). Until `NOTAAS_API_KEY` and `NOTAAS_SCHEMA_CONFIRMED=1` exist, invoices stay `pending` with a
  clear error, never guessed.
