...
**Project:** PatentSonar — USD-priced B2B subscription newsletter on rare-earth-free magnet patents
...
**Operating conclusion for PatentSonar.**
...
1. **Separate outreach domain** (e.g., `patentsonar-intel.com` or `patentsonar-mail.com`) that 301-redirects to the main site. Never send outreach from the newsletter/transactional domain (`patentsonar.ai`/main brand) so that a spam complaint can never poison Postmark delivery of the paid product.
...
- Paddle is the **Merchant of Record**: it is the legal seller to the end customer, collects and remits VAT/GST/sales tax in ~all jurisdictions, handles chargebacks/fraud and issues customer invoices; PatentSonar sells to Paddle (a cross-border B2B supply, reverse-charged in the UK) and receives a net payout. — [How Paddle handles VAT](https://www.paddle.com/help/sell/tax/how-paddle-handles-vat-on-your-behalf) **[excerpt-only]**, [Should I charge Paddle VAT on payouts?](https://www.paddle.com/help/manage/get-paid/should-i-charge-paddle-vattax-for-payouts) **[excerpt-only]**.
