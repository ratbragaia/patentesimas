...
- [ ] Postmark server token + DNS records (`infra/dns-records.md`) ✅ 2026-09-26 (Free plan, domain verified, test
      email delivered, webhooks registered); Paddle ✅ 2026-09-26 (live webhook verified; sandbox catalog created, ids in `ps.plans`); Notaas key stored (homologação confirmation pending); outreach mailbox still pending.
