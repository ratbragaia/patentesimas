# DNS records (main domain: rarefree.com — verify availability at purchase; fallbacks .io / .ai)
...
| TXT | _dmarc | `v=DMARC1; p=quarantine; rua=mailto:dmarc@rarefree.com; fo=1` | DMARC (start `p=none` for 2 weeks, then quarantine) |
| MX | @ | provider of the inbox that receives replies (Google Workspace / Fastmail) | inbound mail |

Outreach uses a **separate domain** (e.g. `rarefree-intel.com`) with its own SPF/DKIM/DMARC and a
