...
#   jobs: status | logs <unit> [n] | env-keys | git-pull | npm-ci | test | migrate | caddy-sync | site-check | agent-health | dns-dmarc | ssh-check | restart [unit] | cli <subcommand...> | sql-count
set -euo pipefail
