...
#        bash infra/cloudflare/dns-upsert.sh --check <TYPE> <NAME>      (read-only: prints the current record or "none")
set -euo pipefail
...
TYPE="${1:?type}"; NAME="${2:?name}"; CONTENT="${3:-}"; TTL="${4:-1}"; PROXIED="${5:-false}"
[ "$CHECK" = 1 ] || [ -n "$CONTENT" ] || { echo "content required"; exit 1; }
