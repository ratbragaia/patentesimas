...
EXISTING=$(curl -sS --max-time 30 "$API?type=$TYPE&name=$FQDN" -H "Authorization: Bearer $CF")
...
if [ -n "$ID" ]; then
  RES=$(curl -sS --max-time 30 -X PUT "$API/$ID" -H "Authorization: Bearer $CF" -H "Content-Type: application/json" --data "$BODY"); ACTION=updated
else
  RES=$(curl -sS --max-time 30 -X POST "$API" -H "Authorization: Bearer $CF" -H "Content-Type: application/json" --data "$BODY"); ACTION=created
fi
printf '%s' "$RES" | python3 -c 'import sys,json
d=json.load(sys.stdin); a=sys.argv[1]
if d.get("success"): r=d["result"]; print(f"{a} {r[\"type\"]} {r[\"name\"]} -> {r[\"content\"]}")
