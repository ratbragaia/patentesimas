...
export function resolveJob(name: string, args: string[] = []): Job | null {
  switch (name) {
    ...
    case "agent-health": return { argv: ["bash", "-lc", "echo '== timers'; sudo systemctl list-timers 'patentsonar-*' --no-pager | head -12; echo '== last agent runs'; for f in $(ls -t /var/log/patentsonar/agent/run-*.log 2>/dev/null | head -5); do echo \"$(basename $f) $(grep -m1 '^== exit' $f || echo 'running/incomplete')\"; done; echo '== failed units'; systemctl list-units 'patentsonar-*' --state=failed --no-pager --no-legend 2>/dev/null || true; echo '== disk'; df -h / | tail -1; echo '== last ingest runs'; cd " + APP_DIR + " && DB=$(grep -E '^SUPABASE_DB_URL=' /etc/patentsonar/env | cut -d= -f2- | tr -d '\"') && psql \"$DB\" -X -q -t -c \"select source, status, window_end, started_at::date from ps.ingest_runs order by started_at desc limit 5\"; echo '== open founder tasks'; psql \"$DB\" -X -q -t -c \"select count(*) from ps.tasks where status='blocked'\""] };
    // Fixed DNS records only (no free-form DNS from the ops channel). DMARC per infra/dns-records.md.
    case "dns-dmarc": return { argv: ["bash", `${APP_DIR}/infra/cloudflare/dns-upsert.sh`, "TXT", "_dmarc", "v=DMARC1; p=none; rua=mailto:dmarc@patentsonar.com; fo=1"] };
    // Which Cloudflare token the VPS holds (id + status via /user/tokens/verify) and whether it can read the zone's DNS. Read-only.
    case "cf-token-check": return { argv: ["bash", "-lc", "CF=$(grep -E '^CLOUDFLARE_API_TOKEN=' /etc/patentsonar/env | head -1 | cut -d= -f2- | tr -d '\"'); Z=$(grep -E '^CLOUDFLARE_ZONE_ID=' /etc/patentsonar/env | head -1 | cut -d= -f2- | tr -d '\"'); A=$(grep -E '^CLOUDFLARE_ACCOUNT_ID=' /etc/patentsonar/env | head -1 | cut -d= -f2- | tr -d '\"'); echo \"token sha256 prefix: $(printf %s \"$CF\" | sha256sum | cut -c1-8)  zone id: ${Z:0:6}…${Z: -4}\"; echo '== verify (account token endpoint, then user token endpoint)'; curl -sS --max-time 20 \"https://api.cloudflare.com/client/v4/accounts/$A/tokens/verify\" -H \"Authorization: Bearer $CF\" | python3 -c 'import sys,json;d=json.load(sys.stdin);print(\"account:\", json.dumps(d.get(\"result\") or d.get(\"errors\")))'; curl -sS --max-time 20 https://api.cloudflare.com/client/v4/user/tokens/verify -H \"Authorization: Bearer $CF\" | python3 -c 'import sys,json;d=json.load(sys.stdin);print(\"user:\", json.dumps(d.get(\"result\") or d.get(\"errors\")))'; echo '== dns read'; U=\"https://api.cloudflare.com/client/v4/zones/$Z/dns_records?type=TXT&name=_dmarc.patentsonar.com\"; printf 'url: %s\\n' \"$U\" | sed 's/zones\\/[0-9a-f]\\{6\\}[0-9a-f]*/zones\\/<zone>/' | cat -A; curl -sS --max-time 20 \"$U\" -H \"Authorization: Bearer $CF\" | python3 -c 'import sys,json;d=json.load(sys.stdin);print(\"ok, records visible:\", len(d.get(\"result\") or [])) if d.get(\"success\") else print(json.dumps(d.get(\"errors\")))'"] };
