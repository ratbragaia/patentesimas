...
export function resolveJob(name: string, args: string[] = []): Job | null {
  switch (name) {
    ...
    case "cf-token-check": return { argv: ["bash", "-lc", `A=$(grep -E '^CLOUDFLARE_ACCOUNT_ID=' /etc/patentsonar/env | head -1 | cut -d= -f2- | tr -d '"'); CF=$(grep -E '^CLOUDFLARE_API_TOKEN=' /etc/patentsonar/env | head -1 | cut -d= -f2- | tr -d '"'); echo '== token'; curl -sS --max-time 20 "https://api.cloudflare.com/client/v4/accounts/$A/tokens/verify" -H "Authorization: Bearer $CF" | python3 -c 'import sys,json;d=json.load(sys.stdin);print(json.dumps(d.get("result") or d.get("errors")))'; echo '== dns read (same code path as dns-dmarc)'; bash ${APP_DIR}/infra/cloudflare/dns-upsert.sh --check TXT _dmarc`] };
    // Where the server physically is (for the privacy policy's hosting disclosure) and basic host facts. Read-only.
    case "host-info": return { argv: ["bash", "-lc", "hostnamectl 2>/dev/null | sed -n '1,6p'; echo '== timezone'; timedatectl show -p Timezone --value 2>/dev/null; echo '== public ip geolocation'; curl -s --max-time 10 https://ipinfo.io/json | python3 -c 'import sys,json;d=json.load(sys.stdin);print({k:d.get(k) for k in (\"ip\",\"city\",\"region\",\"country\",\"org\")})' 2>/dev/null || curl -s --max-time 10 https://ifconfig.co/json | head -c 400"] };
