 ...
 * design "US-D1000000-S" (all off-topic for us, but a row must never abort an ingest); Taiwan invention
 * patents "TW-I844296-B" and utility models "TW-M..." keep their letter too.
 */
export function normalizePublicationNumber(raw: string, countryHint?: string, kindHint?: string | null): string {
  const s = raw.trim().toUpperCase().replace(/\s+/g, "");
  let m = s.match(/^([A-Z]{2})-?((?:PP|RE|D|H|T|X|AI|I|M)?\d+)-?([A-Z]\d?)?$/);
  if (m) {
