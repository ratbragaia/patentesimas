...
export const NICHE = {
  ...
  keywords: [
    ...
    "cerium magnet", "Ce-substituted", "cerium-substituted",
  ],
  // Terms that, when present with a rare-earth CPC and no RE-free keyword, indicate noise.
  noise: ["NdFeB", "Nd2Fe14B", "neodymium iron boron", "samarium cobalt", "SmCo"],
  buckets: {
    ...
    ferrite: ["ferrite"],
    re_lean: ["rare-earth-lean", "rare earth lean", "reduced dysprosium", "dysprosium-free", "heavy rare earth free", "cerium", "ce-substituted"],
    motor_topology: ["reluctance", "wound rotor", "flux barrier", "induction motor"],
...
export function classify(pub: { title?: string | null; abstract?: string | null; cpc_codes: string[] }): string[] | null {
  ...
  const reFreeTerm = terms.some((k) => /rare|dysprosium|cerium|ce-sub/i.test(k));
